...
const CreateInvoice = ({ onSave, onCancel, userSettings, clients = [], invoices = [], editingInvoice }) => {
  ...
  const [client, setClient] = useState({
    id: '', name: '', gstin: '', state: 'Maharashtra',
...
const [settings, setSettings] = useState({
  ...
  useEffect(() => {
    if (editingInvoice) {
      ...
      // Normalize loaded client state
      const loadedClient = editingInvoice.client || {};
      let fixedState = loadedClient.state || 'Maharashtra';
      if (['foreign', 'other'].includes(fixedState.toLowerCase())) fixedState = 'Other';

      setClient({ ...loadedClient, state: fixedState });

      const extractDate = (val) => val?.includes('T') ? val.split('T')[0] : val || '';
      ...
      const finalDatePaid = loadedDatePaid || (editingInvoice.status === 'Paid' ? invoiceDate : '');
  ...
  const handleClientSelect = (clientId) => {
    ...
    if (selected) {
      // FIX: normalize state for export logic
      let fixedState = selected.state || 'Maharashtra';
      if (['foreign', 'other', 'foreign (export)'].includes(fixedState.toLowerCase()))
        fixedState = 'Other';

      setClient({
        ...selected,
        state: fixedState,
        selectedContact: selected.contacts?.length > 0 ? selected.contacts[0] : null
      });
    } else {
      setClient({
        id: '', name: '', gstin: '', state: 'Maharashtra',
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="col-span-2 p-6 space-y-6">
          ...
          {/* ---------- BILLING DETAILS ---------- */}
          <div>
            <h3 className="text-sm font-semibold dark:text-white uppercase tracking-wider mb-4 border-b border-slate-200 pb-2">Bill To</h3>
          ...
          {/* ---------- LINE ITEMS ---------- */}
          <div>
            <h3 className="text-sm font-semibold dark:text-white uppercase tracking-wider mb-4 border-b border-slate-200 pb-2">Items</h3>
            ...
            <div className="space-y-3">
              ...
              <Button variant="ghost" onClick={addItem} icon={Plus} className="w-full mt-2 text-sm border-dashed border border-slate-300">
                Add Line Item
              </Button>
            </div>
          </div>
...
<Card className="p-6 space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <label htmlFor="lut" className="text-sm dark:text-slate-300">Export under LUT</label>
            </div>
          </Card>
